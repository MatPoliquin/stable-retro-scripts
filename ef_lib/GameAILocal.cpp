...
// C API
//=======================================================
extern "C" DllExport void game_ai_lib_init(void* obj_ptr, void* ram_ptr,
                                           int ram_size)
{
    if (obj_ptr)
...
game_ai_lib_think(void* obj_ptr, bool buttons[GAMEAI_MAX_BUTTONS], int player,
                  const void* frame_data, unsigned int frame_width,
                  unsigned int frame_height, unsigned int frame_pitch,
                  unsigned int pixel_format)
{
    if (obj_ptr)
        static_cast<GameAI*>(obj_ptr)->Think(buttons, player, frame_data,
                                             frame_width, frame_height,
                                             frame_pitch, pixel_format);
}

extern "C" DllExport void game_ai_lib_set_show_debug(void* obj_ptr,
                                                     const bool show)
{
    if (obj_ptr)
...
extern "C" DllExport void game_ai_lib_set_debug_log(void* obj_ptr,
                                                    debug_log_t func)
{
    if (obj_ptr)
...
//=======================================================
void GameAILocal::LoadConfig_Player(
    const nlohmann::detail::iter_impl<const nlohmann::json>& player)
{
    for (auto var = player->cbegin(); var != player->cend(); ++var)
    {
        if (var.key() == "models")
        {
            for (auto model = var.value().cbegin(); model != var.value().cend();
                 ++model)
            {
                ...
                RetroModel* load_model =
                    this->LoadModel(modelPath.string().c_str());

                if (models.count(model.key()) == 0)
                {
                    models.insert(std::pair<std::string, RetroModel*>(
                        model.key(), load_model));
                }
            }
        }
    }
