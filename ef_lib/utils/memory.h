...
template <typename T = uint8_t> class MemoryView {
public:
  MemoryView() {}
...
template <typename T> MemoryView<T>::~MemoryView() { close(); }
...
#ifdef _WIN32
  m_mapView =
      CreateFileMapping(reinterpret_cast<HANDLE>(_get_osfhandle(m_backingFd)),
                        0, PAGE_READWRITE, 0, m_size, 0);
  m_buffer = reinterpret_cast<T *>(static_cast<uint8_t *>(
      MapViewOfFile(m_mapView, FILE_MAP_WRITE, 0, 0, m_size)));
#else
  m_buffer = reinterpret_cast<T *>(static_cast<uint8_t *>(mmap(
      nullptr, m_size, PROT_READ | PROT_WRITE, MAP_SHARED, m_backingFd, 0)));
#endif
...
template <typename T> void MemoryView<T>::open(void *buffer, size_t bytes) {
  if (ok()) {
    close();
  }
  m_backingFd = -1;
...
template <typename T> void MemoryView<T>::open(size_t bytes) {
  if (ok()) {
    close();
  }
  m_backingFd = -1;
...
#else
  m_buffer = static_cast<T *>(mmap(nullptr, bytes, PROT_READ | PROT_WRITE,
                                   MAP_ANON | MAP_SHARED, -1, 0));
#endif
}

template <typename T> void MemoryView<T>::open(std::initializer_list<T> list) {
  open(list.size());
  std::copy(list.begin(), list.end(), m_buffer);
}

template <typename T> void MemoryView<T>::close() {
  if (!ok()) {
    return;
  }
  if (m_managed) {
...
template <typename T> bool MemoryView<T>::ok() const {
  return m_buffer && m_size;
}

template <typename T> void MemoryView<T>::clone() {
  if (!ok() || m_managed) {
    return;
  }
...
void MemoryView<T>::clone(const void *buffer, size_t bytes) {
  ...
  T *newBuffer = static_cast<T *>(
      VirtualAlloc(nullptr, bytes, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE));
#else
  T *newBuffer = static_cast<T *>(mmap(nullptr, bytes, PROT_READ | PROT_WRITE,
                                       MAP_ANON | MAP_SHARED, -1, 0));
#endif
...
template <typename T> void MemoryView<T>::clone(const MemoryView<T> &other) {
  clone(static_cast<const void *>(other.m_buffer), other.m_size);
}

template <typename T> T &MemoryView<T>::operator[](size_t index) {
  return m_buffer[index];
}

template <typename T> const T &MemoryView<T>::operator[](size_t index) const {
  return m_buffer[index];
}
...
template <typename T> void *MemoryView<T>::offset(size_t index) {
  return reinterpret_cast<void *>(&m_buffer[index]);
}

template <typename T> const void *MemoryView<T>::offset(size_t index) const {
  return reinterpret_cast<const void *>(&m_buffer[index]);
}

template <typename T> size_t MemoryView<T>::size() const { return m_size; }
...
class MemoryOverlay {
public:
  MemoryOverlay(Endian backing = Endian::NATIVE, Endian real = Endian::NATIVE,
                size_t width = 1);
...
class Variant {
public:
  enum class Type { BOOL, INT, FLOAT, VOID };
...
public:
  ...
  Datum(void *base, size_t offset, const DataType &,
...
public:
  DynamicMemoryView(void *buffer, size_t bytes, const DataType &,
                    const MemoryOverlay & = {});
...
namespace std {
template <> struct hash<Retro::DataType> {
