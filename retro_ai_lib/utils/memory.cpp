...
bool Retro::reduceCompare(Endian a, Endian b) { return reduce(a) == reduce(b); }

DataType::DataType(const char *type)
    : width(type[strlen(type) - 1] - '0'),
      endian(type[0] == '='   ? Endian::NATIVE
             : type[0] == '>' ? (type[1] == '<'   ? Endian::MIXED_BL
                                 : type[1] == '=' ? Endian::MIXED_BN
                                                  : Endian::BIG)
             : type[0] == '<' ? (type[1] == '>'   ? Endian::MIXED_LB
                                 : type[1] == '=' ? Endian::MIXED_LN
                                                  : Endian::LITTLE)
                              : Endian::UNDEF),
  ...
  uint64_t shiftInc = repr == Repr::BCD ? 100 : repr == Repr::LN_BCD ? 10 : 256;
...
DataType::DataType(const string &type) : DataType(type.c_str()) {}

Datum DataType::operator()(void *base) const { return Datum(base, *this); }

Datum DataType::operator()(void *base, size_t offset,
                           const MemoryOverlay &overlay) const {
...
MemoryOverlay::MemoryOverlay(char backing, char real, size_t width)
    : width(width), m_backing({backing, 'u', static_cast<char>('0' + width)}),
      m_real({real, 'u', static_cast<char>('0' + width)}) {}

void *MemoryOverlay::parse(const void *in, size_t offset, void *out,
                           size_t size) const {
...
void MemoryOverlay::unparse(void *out, size_t offset, const void *in,
                            size_t size) const {
  size_t offsetEdge = offset & (width - 1);
...
Variant::Variant(int64_t i) : m_type(Type::INT), m_vi(i) {}

Variant::Variant(double d) : m_type(Type::FLOAT), m_vf(d) {}

Variant::Variant(bool b) : m_type(Type::BOOL), m_vb(b) {}

Variant::operator int64_t() const { return cast<int64_t>(); }

Variant::operator int() const { return cast<int>(); }
...
void Variant::clear() { m_type = Type::VOID; }
...
Datum::Datum(void *base, const DataType &type) : m_base(base), m_type(type) {}

Datum::Datum(void *base, size_t offset, const DataType &type,
             const MemoryOverlay &overlay)
    : m_base(base), m_offset(offset), m_type(type), m_overlay(overlay) {}

Datum::Datum(void *base, const Variable &var, const MemoryOverlay &overlay)
    : m_base(base), m_offset(var.address), m_type(var.type), m_mask(var.mask),
      m_overlay(overlay) {}

Datum::Datum(Variant *variant) : m_type("=i8"), m_variant(variant) {}

Datum &Datum::operator=(int64_t value) {
  if (m_base) {
    if (m_overlay.width > 1 || m_offset) {
      uint8_t fakeBase[16]{};
      m_type.encode(m_overlay.parse(m_base, m_offset,
                                    reinterpret_cast<void *>(fakeBase),
                                    m_type.width),
                    value);
      m_overlay.unparse(m_base, m_offset, reinterpret_cast<void *>(fakeBase),
                        m_type.width);
    } else {
...
Datum::operator int64_t() const {
  ...
  if (m_overlay.width > 1 || m_offset) {
    uint8_t fakeBase[16]{};
    value = m_type.decode(m_overlay.parse(
        m_base, m_offset, reinterpret_cast<void *>(fakeBase), m_type.width));
  } else {
...
DynamicMemoryView::DynamicMemoryView(void *buffer, size_t bytes,
                                     const DataType &dtype,
                                     const MemoryOverlay &overlay)
    : dtype(dtype), overlay(overlay) {
...
int64_t DynamicMemoryView::operator[](size_t offset) const {
  if (overlay.width > 1) {
    uint8_t fakeBase[16]{};
    return dtype.decode(overlay.parse(m_mem.offset(0), offset,
                                      reinterpret_cast<void *>(fakeBase),
                                      dtype.width));
  }
...
bool AddressSpace::ok() const { return m_blocks.size() > 0; }

void AddressSpace::reset() { m_blocks.clear(); }
...
Datum AddressSpace::operator[](const Variable &var) {
  for (auto &kv : m_blocks) {
    ...
    return Datum(kv.second.offset(0),
                 Variable{var.type, var.address - kv.first, var.mask},
                 *m_overlay);
  }
...
uint8_t AddressSpace::operator[](size_t offset) const {
  for (const auto &kv : m_blocks) {
    ...
    uint8_t fakeBase[16]{};
    return s_type.decode(
        m_overlay->parse(kv.second.offset(0), offset - kv.first,
                         reinterpret_cast<void *>(fakeBase), s_type.width));
  }
...
int64_t AddressSpace::operator[](const Variable &var) const {
  for (const auto &kv : m_blocks) {
    ...
    if (m_overlay->width > 1) {
      uint8_t fakeBase[16];
      value = var.type.decode(
          m_overlay->parse(kv.second.offset(0), var.address - kv.first,
                           reinterpret_cast<void *>(fakeBase), var.type.width));
    } else {
