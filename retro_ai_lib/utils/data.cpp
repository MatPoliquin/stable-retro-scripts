...
template <typename T> T find(json::const_reference j, const string &key) {
...
static void setActions(const vector<string> &buttonList,
                       const vector<vector<vector<string>>> &actionsIn,
                       map<int, set<int>> &actions) {
  actions.clear();
...
static unsigned filterAction(unsigned action,
                             const map<int, set<int>> &actions) {
...
bool GameData::load(istream *file) {
  ...
  for (auto var = info->cbegin(); var != info->cend(); ++var) {
    if (var->find("address") == var->cend() ||
        var->find("type") == var->cend()) {
      oldVars.swap(m_vars);
      return false;
    }
...
void GameData::restart() { m_customVars.clear(); }
...
vector<string> GameData::buttons() const { return m_buttons; }

void GameData::setActions(const vector<vector<vector<string>>> &actions) {
  ::setActions(m_buttons, actions, m_actions);
}

map<int, set<int>> GameData::validActions() const { return m_actions; }
...
size_t GameData::numVariables() const { return m_vars.size(); }
...
/*void GameData::deltaSearch(const std::string& name, Operation op, int64_t
reference) { if (m_searches.find(name) == m_searches.cend()) { if
(m_types.size()) { m_searches.emplace(name, Search{ m_types }); } else {
                        m_searches.emplace(name, Search{});
                }
        }
